...
void EngineSystemManager::RegisterSystemDescriptor(
    const EngineSystemDescriptor &descriptor) {
  size_t interfaceHash = descriptor.GetInterfaceHash();

  // Check for duplicate interface registration (single subsystem per interface
  // constraint)
  ...
  if (it != registeredDescriptorsByInterface.end() && !it->second.empty()) {
    throw std::runtime_error(
        "Cannot register multiple subsystems for interface: " +
...
void EngineSystemManager::UnregisterSystemDescriptor(
    const EngineSystemDescriptor &descriptor) {
  size_t interfaceHash = descriptor.GetInterfaceHash();
    ...
    if (!entry->initialized) {
      if (!entry->subsystem->Initialize()) {
        std::cerr << "Failed to initialize subsystem: "
                  << entry->descriptor.GetSystemName() << std::endl;
...
std::vector<EngineSystemDescriptor>
EngineSystemManager::GetRegisteredDescriptors() const {
  std::vector<EngineSystemDescriptor> result;

  for (const auto &[interfaceHash, descriptors] :
...
std::vector<const IEngineSubsystem *>
EngineSystemManager::GetActiveSubsystems() const {
