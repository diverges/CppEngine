...
public:
  ...
  template <typename SystemInterface> SystemInterface *GetSystem();

  template <typename SystemInterface> const SystemInterface *GetSystem() const;

  template <typename SystemInterface> bool HasSystem() const;

  template <typename SystemInterface> void DestroySystem();
...
private:
  ...
  std::unordered_map<size_t, std::unique_ptr<SystemEntry>>
      activeSubsystemsByInterface;
...
template <typename SystemInterface>
SystemInterface *
EngineSystemManager::CreateSystem(const EngineSystemDescriptor &descriptor) {
  size_t interfaceHash = std::type_index(typeid(SystemInterface)).hash_code();

  // Check if system already exists
  if (activeSubsystemsByInterface.find(interfaceHash) !=
      activeSubsystemsByInterface.end()) {
...
bool EngineSystemManager::HasSystem() const {
  size_t interfaceHash = std::type_index(typeid(SystemInterface)).hash_code();
  return activeSubsystemsByInterface.find(interfaceHash) !=
         activeSubsystemsByInterface.end();
}

template <typename SystemInterface> void EngineSystemManager::DestroySystem() {
  size_t interfaceHash = std::type_index(typeid(SystemInterface)).hash_code();
