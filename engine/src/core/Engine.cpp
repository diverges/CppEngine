...
bool EngineConfig::Validate() const {
    ...
    if (openglMajorVersion < 3 || (openglMajorVersion == 3 && openglMinorVersion < 3)) {
...
Engine::Engine(const EngineConfig& config) : m_pImpl(std::make_unique<Impl>(config)) {}
...
bool Engine::Initialize() {
    ...
    windowConfig.resizable = false;  // Prevent window resizing for scripted experience
...
void Engine::Update(double deltaTime) {
    ...
    auto frameDuration =
        std::chrono::duration_cast<std::chrono::microseconds>(currentTime - m_pImpl->lastFrameTime)
            .count();
...
double Engine::GetUptime() const {
    auto currentTime = std::chrono::high_resolution_clock::now();
    auto uptime =
        std::chrono::duration_cast<std::chrono::milliseconds>(currentTime - m_pImpl->startTime)
            .count();
