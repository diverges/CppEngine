...
public:
  ...
  CreateCube(float size = 1.0f, const glm::vec3 &color = glm::vec3(1.0f));

  /**
   * @brief Create a cube with individual face colors
   * @param size Size of the cube
   * @param colors Array of 6 colors for each face (front, back, right, left,
   * top, bottom)
   * @return Unique pointer to created mesh
   */
  static std::unique_ptr<Mesh> CreateColoredCube(float size,
   ...
   */
  static std::unique_ptr<Mesh>
  CreateQuad(float width = 1.0f, float height = 1.0f,
   ...
   */
  static std::unique_ptr<Mesh>
  CreateTriangle(const glm::vec3 &color = glm::vec3(1.0f));
