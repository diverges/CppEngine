...
#include "../../include/AIEngine/graphics/Mesh.hpp"
#include <iostream>
...
namespace AIEngine
{
    ...
    Mesh::Mesh(const std::vector<Vertex> &vertices, const std::vector<uint32_t> &indices)
        : m_vertices(vertices), m_indices(indices), m_vao(0), m_vbo(0), m_ebo(0), m_uploaded(false)
    {
    ...
    Mesh::Mesh(Mesh &&other) noexcept
        : m_vertices(std::move(other.m_vertices)), m_indices(std::move(other.m_indices)), m_vao(other.m_vao), m_vbo(other.m_vbo), m_ebo(other.m_ebo), m_uploaded(other.m_uploaded)
    {
