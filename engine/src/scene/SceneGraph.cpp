...
SceneGraph::FindAllNodesByName(const std::string &name) {
  auto it = m_nameRegistry.find(name);
  return (it != m_nameRegistry.end()) ? it->second : std::vector<SceneNode *>();
}

void SceneGraph::ForEachNode(std::function<void(SceneNode *)> func,
                             bool activeOnly) {
  if (!func || !m_rootNode) {
    return;
  }
...
void SceneGraph::ForEachNode(std::function<void(const SceneNode *)> func,
...
void SceneGraph::TraverseNodes(SceneNode *node,
                               std::function<void(SceneNode *)> func,
                               bool activeOnly) {
...
void SceneGraph::TraverseNodes(const SceneNode *node,
