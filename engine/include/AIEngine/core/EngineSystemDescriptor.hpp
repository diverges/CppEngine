 ...
 *
 * EngineSystemDescriptor encapsulates all the type information and creation logic
 * needed to instantiate a subsystem with its bound provider. It serves as a
 * type-safe factory that maintains the relationship between subsystem interfaces,
 * implementations, and their platform-specific providers.
 *
...
class EngineSystemDescriptor {
   public:
     ...
     *
     * @tparam SubsystemInterface The interface type (must inherit from IEngineSubsystem)
     * @tparam SubsystemImpl The implementation type (must implement SubsystemInterface)
     * @tparam ProviderType The provider type (must inherit from ISubsystemProvider)
     *
     ...
     *
     * Two descriptors are equal if they have the same interface and provider types.
     * System name and other metadata do not affect equality.
     *
     * @param other Descriptor to compare against
     * @return true if descriptors represent the same subsystem+provider combination
     */
