...
TestGame::TestGame(TestGame &&other) noexcept
    : m_engine(std::move(other.m_engine)), m_engineConfig(std::move(other.m_engineConfig)), m_sceneGraph(other.m_sceneGraph), m_initialized(other.m_initialized), m_running(other.m_running), m_exitRequested(other.m_exitRequested), m_lastFrameTime(other.m_lastFrameTime), m_deltaTime(other.m_deltaTime), m_fpsUpdateTimer(other.m_fpsUpdateTimer), m_currentFPS(other.m_currentFPS), m_frameCount(other.m_frameCount), m_testObject(other.m_testObject), m_camera(other.m_camera), m_rootNode(other.m_rootNode)
{
...
AIEngine::EngineConfig TestGame::CreateEngineConfig()
{
    ...
    std::cout << "TestGame: Engine config - " << config.windowWidth << "x" << config.windowHeight
...
void TestGame::SetupCamera()
{
    ...
    auto *cameraTransform = cameraNode->AddComponent<AIEngine::TransformComponent>();
    if (cameraTransform)
    {
...
void TestGame::Update(double deltaTime)
{
    ...
    if (m_testObject)
    {
        auto *transform = m_testObject->GetComponent<AIEngine::TransformComponent>();
        if (transform)
        {
...
void TestGame::PrintDebugInfo()
{
    ...
    if (m_testObject)
    {
        ...
        if (transform)
        {
            ...
            std::cout << "Test Object - Pos: (" << pos.x << ", " << pos.y << ", " << pos.z << ")\n";
            std::cout << "            - Rot: (" << euler.x << ", " << euler.y << ", " << euler.z << ")\n";
        }
    }
